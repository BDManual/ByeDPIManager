...
* Блок **"Настройки ProxiFyre"**
  ...
  * ```Порт``` - указание для ProxiFyre порта, на котором запущен прокси-сервер, в данном случае ByeDPI.
   ...
   >Если по какой-либо причине порт уже занят другим приложением и ByeDPI Manager в логе выдает ошибку</br>
   > ```ByeDPI: bind: 10013``` </br>
   > то для смены порта надо изменить его не только в поле "Порт" настройках ProxiFyre, но и также добавить аргумент </br>
   > ```-p<порт>```</br>
